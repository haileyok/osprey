...
class OzoneClient:
    ...
    def add_or_remove_label(
        ...
        logger.error(f'Failed to emit label event after {_MAX_RETRIES + 1} attempts: {last_exc}')

    def add_did_to_list(self, did: str, list_uri: str):
        if self._session is None:
            logger.error('Bluesky session not initialized, cannot add DID to list')
            return
        ...
        logger.error(f'Failed to create listitem record after {_MAX_RETRIES + 1} attempts: {last_exc}')
    ...
    def get_did_labels(self, did: str) -> EntityLabels:
        ...
        if self._session is None:
            logger.error('Bluesky session not initialized, cannot fetch labels')
        ...
        try:
            ...
            if e.response is not None and e.response.status_code == 400:
                logger.debug(f'Repo not found for {did}, returning empty labels')
                return create_empty_entity_labels()
